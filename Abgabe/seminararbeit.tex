...
\begin{figure}[ht]
 ...
 \label{img:armijo}
\end{figure}
Die gestrichelte Linie in Abbildung \ref{img:armijo} verdeutlicht die Armijo-Bedinung. Die Funktionswerte, die unter ihr liegen, erfüllen die Bedingung, die Funktionswerte darüber jedoch nicht. Die Armijo-Bedingung alleine reicht aber nicht um eine Kovergenz zu garantieren, denn ein genügend kleiner Schritt erfüllt die Bedingung immer. Dazu eignet sich die Krümmungs-Bedingung.
\begin{equation}
\label{eq:curvature}
f'(x_n+\alpha*s_n)^T s_n \ge c_2 f'(x_n)^T s_n
\end{equation}
Diese Bedingung erzwingt, dass die Funktion im gewählten Punkt nicht mehr so stark abnimmt wie im Punkt $x_n$. Wenn nämlich die Steigung in einem Punkt noch stark negativ ist, macht es Sinn noch weiter in diese Richtung zu gehen. Die Bedingungen (\ref{eq:armijo}) und (\ref{eq:curvature}) zusammen nenn man die Wolfe-Bedingungen. Fügt man zur linken und rechten Seite von Bedingung (\ref{eq:curvature}) noch Beträge hinzu, dann spricht man von den starken Wolfe-Bedingungen. Wir verwenden jedoch diese Variante, denn die starken Wolfe-Bedingungen eignen sich nicht für nicht-glatte Funktionen \cite{DBLP:journals/mp/LewisO13}. Ein recht einfacher und für nicht-glatte Funktionen geeigneter Algorithmus um einen Punkt zu finden, der die Bedingungen (\ref{eq:armijo}) und (\ref{eq:curvature}) erfüllt, ist in Abbildung (\ref{alg:linesearch}) beschrieben. Sobald ein Punkt gefunden wurde, der die Armijo-Bedingung verletzt, setzt er die gesteste Schirtweite als obere Schranke und beginnt das gefundene Intervall zu halbieren bis ein Punkt gefunden wurde, der zusätzlich die Krümmungs-Bedingung erfüllt.
...
\begin{figure}[ht]
 ...
 \begin{enumerate}
 	...
 	\item Wiederhole:
 	\begin{enumerate}
 	\item Wenn Bedingung (\ref{eq:armijo}) verletzt ist, setze $v=\alpha$
 	\item Ansonsten wenn Bedingung (\ref{eq:curvature}) verletzt ist, setzte $u=\alpha$.
...
Für das Packproblem haben wir Barriere-Verfahren als weniger gut geeignet eingestuft. Barriere-Verfahren arbeiten mit Bestrafung- Termen die schon unendlich groß werden wenn die Lösung sich einer ungültigen Lösung annähert. So wird eine Suche durch den Raum der gültigen Lösungen ausgeführt. Im Packproblem sind jedoch gerade die Lösungen optimal, bei denen sich zwei Polygone berühren, also durch die Barriere-Terme schon unendlich stark bestraft werden. Das Penalty-Verfahren funktioniert ebenfalls mit Bestrafungs-Termen in der Zielfunktion, welche jedoch erst Steigen wenn die Nebenbedingung auch tatsächlich verletzt ist. Bei beiden Verfahren gibt es jedoch numersiche Schwierigkeiten \cite{nocedal1999numerical}. Das augmentierte Lagrange-Verfahren basiert auf dem Penalty-Verfahren fügt jedoch noch zusätzliche Terme hinzu die, die numerische Stabilität verbessern.

\subsection{Penalty-Verfahren}
